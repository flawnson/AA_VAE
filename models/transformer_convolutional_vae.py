...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, channels, dropout=0.1, kernel_size=3):
        ...
        self.mutate = nn.Sequential(
            ConvolutionalBlock(in_c=channels, out_c=channels, padded=True, kernel_size=kernel_size),
            ConvolutionalBlock(in_c=channels, out_c=channels, padded=True, kernel_size=kernel_size))
...
class TransformerModel(nn.Module):
    def __init__(self, model_config, h_dim, z_dim, input_size, device, embeddings_static, requires_grad=True):
        ...
        kernel_dimension = model_config["feed_forward"]
        ...
        encoder_layers = TransformerEncoderLayer(d_model=self.moving_dimension, nhead=nheads, channels=self.channels,
                                                 kernel_size=kernel_dimension)
        self.transformer_encoder = TransformerLayer(encoder_layers, layers)

        decoder_layer = TransformerEncoderLayer(self.moving_dimension, nhead=nheads, channels=self.channels,
                                                kernel_size=kernel_dimension)
